...
\end{equation}
where, rank of $\bm{M}$ is equal to the number of positive singular values.
...
\fig{Images/sf_b}{width =\textwidth,height=12cm}{$\Delta B$ without feedback loop in sensor position 8x (top) and the Allan standard deviation for that $\Delta B$ (bottom). Red horizontal lines represent $\overline{B_l}(\tau)$ for 8 sub-samples over integration time $\tau=64~s$, and vertical dashed lines indicate the upper limit of each sub sample. The Allan deviation of $\Delta B$ in for different taus are shown in bottom graph.. \label{fig:sf_mod}}{The Allan deviation.}
