...
Color map of diagonal matrix $\bm{\Sigma}$ found by SVD of $\bm{M}$ for 18 sensors and 6 coils in nT/A. $\bm{\Sigma}$ is positive square roots of the non-negative eigenvalues of $\bm{M^T}\bm{M}$ in sensors$\times$coils dimension. \label{fig:diag_patch_6c}}{Diagonal Matrix for 6 coils.}
...
We then tried to understand the reason of being the minimum diagonal element to be zero by exploiting right singlur vectors $\bm{V^T}$.
