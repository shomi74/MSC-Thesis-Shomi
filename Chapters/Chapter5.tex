...
and then discussed regularization by random fluctuations in
...
e.g.~Eqs.~(\ref{eq:minvR}), (\ref{eq:del_I}), and (\ref{eq:I})).  If
...
However, this increase in current noise does not noticable increase
the noise on the field measurements, which is similar in both cases.
...
generated field perturbations has been discussed.  The purpose of this
...
$\bm{M^{-1}}$=$\sim 51$ which is by definition the same as the
...
A sample of results was shown in Table~\ref{table:flux_pos}.  I think
...
from the definition of Tikhonov regularization~\cite{bib:wikipedia}.
...
$|\Delta\vec{I}-\bm{M}^{-1}\vec{B}|$, which is analogous to
...
introduces another constraint that the modulus of the vector
$|\Delta\vec{I}|$ simultaneously be minimized.  In
Ref.~\cite{bib:wikipedia}, this is also referred to as L2
minimization.  In a way this is equivalent to the compromise being
...
off against one another in order prevent large current fluctuations
from occurring.
