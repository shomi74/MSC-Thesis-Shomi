...
\fig{Images/filtering}{width = \textwidth}{Filtering effect on the data taken for magnetic field compensation. Vertical axis represent $\Delta$B (see Eq.~(\ref{eq:del_B})) found from sensor '1x' without filter in (a), with Bartington$'$s SCU1 (Signal Conditioning Unit) in (b) and with our filter in (c). For postion of the sensor see Fig.~\ref{fig: coil}. Only '1x' is shown instead of all 14 sensors including the control sensors for simplicity. The 'ON' and 'OFF' vertical dashed lines indicate the time of the perturbation coil being turned 'ON' and 'OFF' respectively.  The current supplied on perturbation coil was 100 mA. The resolution index 1 (see Table 3.4) was used. The loop sampling frequency  for (a), (b) and (c) are shown in Hz. \label{fig:filtering}}{Filtering effect on the data taken for magnetic field compensation}
