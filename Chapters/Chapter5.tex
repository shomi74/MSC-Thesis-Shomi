...
\fig{Images/pi_comp}{width = \textwidth,height=12cm}{(a), and (c) Currents in coil $C_z^+$, and (b), and (d) field change $\Delta B$ for sensor position 1x. The fluxgate positions are 1, 3, 6 and 8.  The feedback parameters are $k_c^p=0.0$, $k_c^i=1.0$ at (a), and $k_c^p=1.0$, $k_c^i=0.0$ at (b) .  The current supplied to the perturbation coil was 10  mA when switch on. Resolution index 1 with 50 averages per cycle were used.
\label{fig:style_of_pi}}{Comparison between old and new PI .}
