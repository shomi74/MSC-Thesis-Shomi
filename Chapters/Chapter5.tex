...
There are four layers of shields have been used for passive shielding in this prototype (see Section~\ref{sec:shield}). The active compensation effect has been seen with outermost shield and no shield. The results are shown in Fig.~\ref{fig:shield_noShield} where the currents (left) that are being sent to the coils ($C_x^\pm$, $C_y^\pm$ and $C_z^\pm$) for filed change $\Delta$B (right) found by Eq.~(\ref{eq:del_B}) in fluxgate positions 1, 3, 6 and 8 excluding two control sensors with outermost passive shield in Fig.~\ref{fig:shield_noShield}\textcolor{blue}{(a)} and without shield in Fig.~\ref{fig:shield_noShield}\textcolor{blue}{(b)}. It is seen that the coil currents (left) and $\Delta$B (right) graphs have similar pattern which clears our confusion about the impact of shield on current unsettle problem.
