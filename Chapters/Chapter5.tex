...
\fig{Images/p25p125}{width=\textwidth,height=14cm}{Currents (left) in all six coils ($C_x^\pm$, $C_y^\pm$ and $C_z^\pm$) and field change $\Delta$B (right) for 12 fluxgate axes with fluxgate positions 1, 3, 6 and 8.  (a) $k_c^p=0.25$, (b) $k_c^p=1.25$.  In both cases $k_c^i=0$.  Grey curves denote the uncompensated $\Delta B$ whereas colors indicate the compensated values with one color denoting each axis.  Vertical dashed lines indicate the perturbation coil being turned on and off.\label{fig:p_pi}}{Coil currents with drift $\Delta$B for different values of $k_c^p$ only.}
...
of $r$ are presented in Section~\ref{sec:r}.
...
\fig{Images/i125}{width=\textwidth,height =7cm}{Currents (left) in all six coils with magnetic field $\Delta$B (right) at 12 sensor positions with fluxgate positions 1, 3, 6 and 8 for $k_c^i$=1.25 with $k_c^p=0$.  Grey curves denote the uncompensated field while color curves denote the measured field.  Vertical dashed lines indicate the perturbation coil being turned on and off.\label{fig:i_pi}}{Coil currents with drift $\Delta$B for $k_c^i$ only.}
...
where it is clear the the fields do not drift with such long
...
and it is discussed further in Section~\ref{sec:5coil}.  But at the
...
compensation, but that it gives rise to slowly-varying currents that
...
Section~\ref{something-in-chapter4} were done.
...
In Section~\ref{sec:simulation}, simulation results are presented and
...
matrix (Section~\ref{sec:condnumber}), we eventually determined that
