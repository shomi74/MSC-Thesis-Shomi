...
$I^{n+1}$ which is based on the current determined by the previous
step $I^n$.  The equation to be used to find the next current in the
new algorithm is
\begin{equation}\label{eq:I_raw}
    I^{n+1}= I^n+M^{-1} (B_{\rm setpoint}-B_{\rm
    measure}^n)=I^n+M^{-1} \Delta B^n
\end{equation}
which is taken directly from Eq.~(28) in Section~5.5 of
Ref.~\cite{rawlik}.  In this equation, $\Delta B^n$ is a vector of
...
control (12 in my case).  The quantity $M^{-1}$ is the Moore-Penrose
pseudoinverse (a $6\times 12$ non-square matrix, in my case).  The
quantity $I^n$ are a vector of currents (dimension 6, in my case).
...
presented in Eqs.~(\ref{someq} 4.3 and 4.4).  Naively the main
...
Eq.~(\ref{someq}) on the other hand, references the next current to
the initial current, and implements two control terms: a proportional
term which appears similar to the error term containing $M^{-1}$
...
In the case of $k^p=0$ and $k^i=1$, the PI feedback algorithm in Eq.~(\ref{fixme}, 4.3, 4.4) can be written as
\begin{equation}\label{eq:I_raw_eq}
    I^{n+1}= I^0+\sum_{i=0}^n M^{-1}\Delta B^i
\end{equation}
...
\begin{equation}\label{eq:nminus1}
    I^{n}= I^0+\sum_{i=0}^{n-1} M^{-1}\Delta B^i.
\end{equation}
...
\begin{split}
I^{n+1}&=I^0+\sum_{i=0}^n M^{-1}\Delta B^i\\
&=I^0+\sum_{i=0}^{n-1}M^{-1}\Delta B^i+ M^{-1}\Delta B^n\\
&=I^n+ M^{-1}\Delta B^n
\end{split}
\end{equation}
...
PI parameters being restricted to the particular values $k^p=0$ and
$k^i=1$.  My recommendation would be to keep these as tunable
...
$k^p=0$ and $k^i=1$ (integral control) does seem to be fairly optimal
...
$n+1$ step
\begin{equation}\label{eq:I_raw_delay}
    I^{n+1}= I^{n-2}+M^{-1} \Delta B^{n-2}
\end{equation}
(which is Eq.~(30) of Ref.~\cite{rawlik}).  This could also be
...
\fig{Images/v2}{width = \textwidth}{
Color map of $\bm{\Sigma}$ found by SVD of $\bm{M}$ shown in Fig.~\ref{fig:m} for 12 sensors and 6 coils in nT/A. $\bm{\Sigma}$ is positive square roots of the non-negative eigenvalues of $\bm{M^T}\bm{M}$ in sensors$\times$coils dimension. \label{fig:v2}}{Square root of eigenvalues of $\bm{M^T}\bm{M}$ and $\bm{M}\bm{M^T}$ for 6 coils.}

\fig{Images/wt}{width = \textwidth}{
Color map of $\bm{W^T}$ found by SVD of $\bm{M}$ shown in Fig.~\ref{fig:m} in A. $\bm{W^T}$ is orthonormal eigenvectors of $\bm{M^T}\bm{M}$ in coils$\times$coils dimension for 6 coil currents. \label{fig:wt}}{Orthonormal eigenvectors of $\bm{M^T}\bm{M}$ for 6 coils.}

\fig{Images/v5c}{width = \textwidth}{Color map of $\bm{\Sigma}$ for 12 sensors and 5 coils in nT/A. $\bm{\Sigma}$ is positive square roots of the non-negative eigenvalues of $\bm{M^T}\bm{M}$ in sensors$\times$coils dimension. \label{fig:v5c}}{Square root of eigenvalues of $\bm{M^T}\bm{M}$ and $\bm{M}\bm{M^T}$ for 5 coils.}

\fig{Images/wt5c}{width = \textwidth}{Color map of $\bm{W^T}$ for 5 coil currents in A. $\bm{W^T}$ is orthonormal eigenvectors of $\bm{M^T}\bm{M}$ in coils$\times$coils dimension. \label{fig:wt5c}}{Orthonormal eigenvectors of $\bm{M^T}\bm{M}$ for 5 coils.}
...
\fig{Images/coil_pseudo2}{width =\textwidth,height=12cm}{
