...
to use the fastest sampling frequency of the ADC.  As can be see in
that Section, using the filter, we successfuly reduced the response
...
response when no magnetic shields are within the coil cube.  When the
...
Fig.~\ref{fig:5.7} is that the connection of the coils to current
...
We were still in dark about current unsettle problem. So we thought
simulating the prototype would confirm whether we were getting the
correct current graph. We first made a simulation without PI control
algorithm to compare the experimental change in coil currents due to
certain perturbation with the simulation ones (Eq.~(\ref{eq:del_I}))
and the results were agreed except some sign issues. But we wanted to
see simulated coil currents in time due to PI control algorithm
(Eq.~(\ref{eq:I})). My supervisor made a simple one dimensional PI
simulation in time in Python programming language which later I used
to make a fully functional multi-dimensional PI simulation. The
simulation process has been divided into three groups. They are
