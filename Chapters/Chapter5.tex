...
We built some analog filters which were discussed in Chapter~\ref{ch:amcP}. Goal of the filter was to remove high-frequency noise (low-pass Butterworth with 10~Hz corner frequency). This would allow the ADC to operate with less averaging, reducing its effective sampling time (denoted by the 'resolution index' as shown in Table~\ref{table:t7freq}). We show the effectiveness in this section. The section starts with defining the sampling frequency and how we first consider building an analog filter in the first place. Then it shows the effect improving the response time on the prototype. Finally, the Section wraps up with some comparison in
...
\fig{Images/filtering}{width = \textwidth}{Filtering effect on the magnetic field signals measured by 14 fluxgate sensors including the control sensors placed at 1, 3 , 6, 8 and center (see Fig.~\ref{fig: coil}). For simlicity only postion '1x' is shown. Vertical axis represent $\Delta$B (see Eq.~(\ref{eq:del_B})) found from the measurement of sensor '1x' without filter in (a), with Bartington$'$s SCU1 (Signal Conditioning Unit) in (b) and with our filter in (c). For postion of the sensor see Fig.~\ref{fig: coil}. The 'ON' and 'OFF' vertical dashed lines indicate the time of the perturbation coil being turned 'ON' and 'OFF' respectively.  The current supplied on perturbation coil was 100 mA. The resolution index 1 (see Table~\ref{table:t7freq}) was used. The loop sampling frequency  for (a), (b) and (c) are shown in Hz. \label{fig:filtering}}{Filtering effect on the magnetic field signals.}
...
\item In simulation a set of tweleve $B_s^\text{rand}$ has been generated by normal distrbution around zero with stnadrd devaiation being 1.5 nT.
\item In experiment  set of tweleve $B_s^\text{rand}$ has been generated by magnetic field signals from fluxgate positions at 1, 3 , 6 and 8 using Eq.~(\ref{eq:del_B}). For postion of the fluxgates plese see Fig.~\ref{fig: coil}.
\end{itemize}


In Section~\ref{sec:inv}, we have already talked about how the
regularization parameter $r$ came into effect. Then, in
Section~\ref{sec:mont}, a Monte Carlo simulation method has been
described which has been taken from Ref.~\cite{bea} where a suitable
value of $r$ has been found which will give the best compromise
between the magnetic field fluctuation and coil current
fluctuation. In this Section, we have run the model in experimental
setup in order to verify the simulation method.
$B_s^{\text{rand}}$ will be replaced by
\begin{equation}\label{eq:B_coils-exp}
    \Delta B_s^{\text{exp}}(r) =B_{\text{uncomp}}^{\text{max}}(r)-B_{\text{uncomp}}^{\text{min}}(r)
\end{equation}

Here, Eq.~(\ref{eq:B_coils-sim}) replaced by
\begin{equation}\label{eq:B_coils-exp}
...
\fig{Images/mont_exp}{width = \textwidth}{p127, 100mA stimulus,p1,Simulation result for one set of $B_s^{\text{rand}}$ for the Fig.~\ref{fig:Isim}, Fig.~\ref{fig:fluc-sim} and Fig.~\ref{fig:I-fluc} are shown in (a), (b) and (c) and corresponding experimental results are in (d), (e) and (f). For the description of the figures see the text in Fig.~\ref{fig:Isim}, Fig.~\ref{fig:fluc-sim} and Fig.~\ref{fig:I-fluc}. For position of fluxgate sensors see Fig.~\ref{fig: coil}.\label{fig:mont_comp}}{Short}
...
Here, the effect of changing proportional gain term (P) or $k_c^p$ of
Eq.~(\ref{eq:I}) will be discussed.
...
\fig{Images/mont_p}{width = \textwidth}{p127, 100mA stimulus,p1,Simulation result for one set of $B_s^{\text{rand}}$ for the Fig.~\ref{fig:Isim}, Fig.~\ref{fig:fluc-sim} and Fig.~\ref{fig:I-fluc} are shown in (a), (b) and (c) and corresponding experimental results are in (d), (e) and (f). For the description of the figures see the text in Fig.~\ref{fig:Isim}, Fig.~\ref{fig:fluc-sim} and Fig.~\ref{fig:I-fluc}. For position of fluxgate sensors see Fig.~\ref{fig: coil}.\label{fig:mont_p}}{Short}
...
The effect of changing $k_c^p$ has been shown in Fig.~\ref{fig:p_pi} where the currents (left) that are being sent to the coils ($C_x^\pm$, $C_y^\pm$ and $C_z^\pm$) for drift $\Delta$B found by Eq.~(\ref{eq:del_B}) for all 12 sensor positions excluding two control sensors. 
\begin{itemize}
...
%     \label{fig:i_pi}
% \end{figure}
\fig{Images/mont_i}{width = \textwidth}{p127, 100mA stimulus,p0.7,r3.5,Simulation result for one set of $B_s^{\text{rand}}$ for the Fig.~\ref{fig:Isim}, Fig.~\ref{fig:fluc-sim} and Fig.~\ref{fig:I-fluc} are shown in (a), (b) and (c) and corresponding experimental results are in (d), (e) and (f). For the description of the figures see the text in Fig.~\ref{fig:Isim}, Fig.~\ref{fig:fluc-sim} and Fig.~\ref{fig:I-fluc}. For position of fluxgate sensors see Fig.~\ref{fig: coil}.\label{fig:mont_i}}{Short}

\FloatBarrier
The effect of changing $k_c^i$ has been shown in Fig.~\ref{fig:i_pi} where the currents (left) that are being sent to the coils ($C_x^\pm$, $C_y^\pm$ and $C_z^\pm$) for drift $\Delta$B found by Eq.~(\ref{eq:del_B}) in all 12 sensor postions excluding two control sensors. 
...
For understating the system response time and the overshoot effect, two similar figures like in Fig.~\ref{fig:i_pi} with $k_c^i$=1.0 and $k_c^i$=1.25 respectively have been generated. From those simulated figures, the $\Delta$B graphs have been zoomed in and shown in Fig.~\ref{fig:i_pi_zoom}. It is seen that the error level is $\sim$3.23 nT  at 25-10=15 s (the perturbation is turned 'ON' at 10 s) for $k_c^i$=1.0  as seen in Fig.~\ref{fig:i_pi_zoom}\textcolor{blue}{(a)} and it will be less for increasing value of $k_c^i$. It is also seen from the Fig.~\ref{fig:i_pi_zoom}\textcolor{blue}{(b)} that there is an overshoot in the error level before it settles in.
\begin{itemize}
    \item message from the Fig.~\ref{fig:i_pi_zoom} ?
\end{itemize}

\doublefig{Images/i100_33_zoom}{width =\textwidth,height =8cm}{at $k_c^i$=1.0 \label{fig:shield_opera2}}{Images/i125_33_zoom}{width = \textwidth,height =8cm}{at $k_c^i$=1.25 \label{fig:noShield_opera}}{{Simulated zoomed version of the drift $\Delta$B similar to right side of Fig.~\ref{fig:i_pi} for all 12 sensor positions with fluxgates in 1, 3, 6 and 6 for different values of $k_c^i$ with $k_c^p$ ( see Eq.~(\ref{eq:I}) ) being zero. For position of coils and sensors see Fig.~\ref{fig: coil}. For description about simulation see Section~\ref{sec:pSim}. The red vertical dashed line indicates the time of the perturbation coil being turned 'ON'. A time delay of 0.145 s was used to match the experimental loop sampling frequency of $\sim$6.5 Hz. \label{fig:i_pi_zoom}}}{Simulated zoomed version of the drift $\Delta$B.}
% \begin{figure}[!htb]
%     \begin{subfigure}{.5\linewidth}
%         \centering
...
Here, first the P and I term have been tuned following the discussion on Section~\ref{sec:tune} which has generated $k_c^p$=0.60 and $k_c^i$=0.37 . The results by applying $k_c^p$ and $k_c^i$ as those tuned values are shown in Fig.~\ref{fig:tuned_vs_i}.
\begin{itemize}
    \item message from the Fig.~\ref{fig:tuned_vs_i} ?
    \item do i have to compare with I term only graph? Then we can use something like following.
\end{itemize}
...
\fig{Images/p60i368}{width = \textwidth,height=7cm}{Currents (left vertical axis) in all six coil sides ($C_x^\pm$, $C_y^\pm$ and $C_z^\pm$) with drift $\Delta$B (right vertical axis) at 12 sensor positions with fluxgate positions 1, 3, 6 and 8 for $k_c^p$=0.6 and $k_c^i$=0.37 (see Eq.~(\ref{eq:I})). For position of coils and fluxgates see Fig.~\ref{fig: coil}. All grey color curves in $\Delta$B graph denote the drift in signal that would have been without the compensation while all other color curves denote the actual drift in signal at all 12 sensor positions found by Eq.~(\ref{eq:del_B}). The 'ON' and 'OFF' vertical dashed lines indicate the time of the perturbation coil being turned 'ON' and 'OFF' respectively. The current supplied on perturbation coil was 10  mA. The resolution index 1 (see Table~\ref{table:t7freq}) with 50 no of averages per measurement was used. The loop sampling frequency was found to be $\sim$6.5 Hz.\label{fig:tuned_vs_i}}{Coil currents with drift $\Delta$B for $k_c^p$ and $k_c^i$ combinely.}
...
\fig{Images/pi_i_mCond30}{width = \textwidth,height=14cm}{Currents (left vertical axis) in all six coil sides ($C_x^\pm$, $C_y^\pm$ and $C_z^\pm$) with drift $\Delta$B (right vertical axis) at 12 sensor positions with fluxgate positions 1, 3, 6 and 8 for for $k_c^p$=0.6 and $k_c^i$=0.37 (see Eq.~(\ref{eq:I})) in (a) and for $k_c^p$=0.0 and $k_c^i$=0.37 (see Eq.~(\ref{eq:I})). For position of coils and fluxgates see Fig.~\ref{fig: coil}. All grey color curves in both $\Delta$B graphs denote the drift in signal that would have been without the compensation while all other color curves denote the actual drift in signal at all 12 sensor positions found by Eq.~(\ref{eq:del_B}). The 'ON' and 'OFF' vertical dashed lines indicate the time of the perturbation coil being turned 'ON' and 'OFF' respectively. The current supplied on perturbation coil was 10  mA. The resolution index 1 (see Table~\ref{table:t7freq}) with 50 no of averages per measurement was used. The loop sampling frequency was found to be $\sim$6.5 Hz.\label{fig:pi_i_mCond30}}{Coil currents with drift $\Delta$B for different values of $k_c^p$ only.}
...
The discussion on Section~\ref{sec:pi_behave} suggests that I term
...
%     \label{fig:r_pi}
% \end{figure}
\fig{Images/mont_pr}{width = \textwidth}{p127, 100mA stimulus,p0.7,r3.5,Simulation result for one set of $B_s^{\text{rand}}$ for the Fig.~\ref{fig:Isim}, Fig.~\ref{fig:fluc-sim} and Fig.~\ref{fig:I-fluc} are shown in (a), (b) and (c) and corresponding experimental results are in (d), (e) and (f). For the description of the figures see the text in Fig.~\ref{fig:Isim}, Fig.~\ref{fig:fluc-sim} and Fig.~\ref{fig:I-fluc}. For position of fluxgate sensors see Fig.~\ref{fig: coil}.\label{fig:mont_pr}}{Short}
\FloatBarrier
...
\fig{Images/sf6sr}{width = \textwidth}{p127, 100mA stimulus,p0.7,r3.5,Simulation result for one set of $B_s^{\text{rand}}$ for the Fig.~\ref{fig:Isim}, Fig.~\ref{fig:fluc-sim} and Fig.~\ref{fig:I-fluc} are shown in (a), (b) and (c) and corresponding experimental results are in (d), (e) and (f). For the description of the figures see the text in Fig.~\ref{fig:Isim}, Fig.~\ref{fig:fluc-sim} and Fig.~\ref{fig:I-fluc}. For position of fluxgate sensors see Fig.~\ref{fig: coil}.\label{fig:adev}}{Short}

\fig{Images/center}{width = \textwidth}{p127, 100mA stimulus,p0.7,r3.5,Simulation result for one set of $B_s^{\text{rand}}$ for the Fig.~\ref{fig:Isim}, Fig.~\ref{fig:fluc-sim} and Fig.~\ref{fig:I-fluc} are shown in (a), (b) and (c) and corresponding experimental results are in (d), (e) and (f). For the description of the figures see the text in Fig.~\ref{fig:Isim}, Fig.~\ref{fig:fluc-sim} and Fig.~\ref{fig:I-fluc}. For position of fluxgate sensors see Fig.~\ref{fig: coil}.\label{fig:center}}{Short}