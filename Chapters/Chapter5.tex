...
The code generated coil geometries were very highly idealized.
...
much like the studies presented in Table~\ref{table:flux-pos}.

\textcolor{red}{Is Fig.~\ref{fig:matrix_patch_6c} required ????}

\fig{Images/matrix_patch_6c}{width = \textwidth}{
Color map of $M_{sc}$ elements for 24 fluxgate axes and 6 coils.\label{fig:matrix_patch_6c}}{Matrix for 6 coils.}
...
component of the $\Sigma_{nn}$ where $n=6$ is limited by the number
of coils. The key problem with the matrix $\bm{\Sigma}$ was that the $\Sigma_{nn}$ 
matrix element was always zero.  Changing the geometry (coils or fluxgates) would 
alter the other singular values, but would never make the smallest singular
...
The right singular vectors are the orthonormal eigenvectors of $\bm{M^T}\bm{M}$.

\fig{Images/Right_patch_6c}{width = \textwidth}{Color map of orthogonal matrix $\bm{V^T}$ found by SVD of $\bm{M}$ for 24 fluxgate axes and 6 coils. \label{fig:right_patch_6c}}{Orthogonal matrix $\bm{V^T}$ for 6 coils.}
...
Color map of $M_{sc}$ elements for 24 fluxgate axes and 5 coils.\label{fig:matrix_patch_5c}}{Matrix for 5 coils.}
