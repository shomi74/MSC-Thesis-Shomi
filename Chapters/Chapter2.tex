...
A particularly challenging aspect is that the TUCAN nEDM experiment will be in fringe field of TRIUMF cyclotron, at a location where the fringe could be as large as eight times the magnetic field of the earth ($\sim376~\mu$T) and $\sim100~\mu$T/m gradients as well as much large changes due to external magnetic sources such as nearby beam line magnets or the movement of large magnetic objects such as crane. During quiet times  $\sim100$~nT fluctuations  are observed but the fluctuations could be as large as  $\sim16~\mu$T due to movement of 50-ton crane. The information are based on the report summarizing the ambient magnetic field measurements by fluxgate magnetometer between May and August 2012 in the Meson Hall at TRIUMF by Ref.~\cite{sarte}. The magnetic field information will be quantified in near future as new fluxgates are being setup in the Meson Hall~\cite{beapriv}.
...
The compensation coils of the AMC system will reduce $\sim16~\mu$T fluctuations to $\sim1~\mu$T$-100$~nT in the 10 Hz to DC frequency range in all three directions by supplying currents to the coils  where the fluctuations will be measured by array of fluxgate sensors in a continuous feedback loop. Every channel  going through the feedback loop must be sampled faster than goal correction rate which is 6~Hz. The goal correction rate is set by :
...
The fluxgate sensors will be placed near the compensation coils and the passive shields.
Other possible applications of the AMC system are :
\begin{enumerate}
    \item {\bf static field reduction.} At large fields, saturation of the passive magnetic shielding system can be a concern, which would seriously impact its effectiveness.
    \item {\bf facilitate opening the door.} When accessing the experiment, the door to the MSR must be opened. If presented with a large external field, the innermost layer of the passive shielding system could themselves become magnetized, necessitating degaussing and additional experimental down time. 
\end{enumerate}
...
$B_1$-coil ($\sim$30~Hz) is used in the Ramsey resonance method for $\pi$/2 spin re-orientation. AC coils will apply the $\pi$/2 pulses for the UCN and comagnetometer species, to initiate free spin precession. UCN need second $\pi$/2 pulse to complete the Ramsey sequence. Then they are drained and their specifications are measured along the quantization axis.
...
The overall objective of this thesis was the development of a prototype AMC system for TUCAN nEDM experiment. The prototype was set up at the University of Winnipeg and successfully tested for one dimensional control by Ref.~\cite{liam_presentation}. I have reproduced the one dimensional result and compared successfully with the result of Ref.~\cite{liam_presentation}. Then I successfully implemented the multi-dimensional control system for the prototype based on the works of Refs.~\cite{bea,lins}. To meet the design goal of 6~Hz and reduce high frequency noise, I have built $4^{th}$-order low-pass Butterworth filter. To understand the prototype and solve various problems related to current drifting, I have made simulation of the system integrating PI feedback control algorithm. I have also followed the works of Ref.~\cite{rawlik} after it was published in 2018. The prototype is now running successfully with multi-dimensional control.
